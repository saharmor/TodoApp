    ...
    setUser((prevUser) => {
      ...
      prevUser.tasks.forEach((task) => {
        if (multipleSelectedTasks.includes(task.id)) {
          ...
          updated.push({ ...task, done: true, lastSave: new Date() });

          // if recurring and we just completed it, generate next occurrence
          if (wasNotDone && task.recurrence) {
            newOccurrences.push({
              ...task,
              id: generateUUID(),
              done: false,
